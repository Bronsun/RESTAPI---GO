...
func testUserToManyUserLinks(t *testing.T) {
	...
	randomize.Struct(seed, &b, userLinkDBTypes, false, userLinkColumnsWithDefault...)
	randomize.Struct(seed, &c, userLinkDBTypes, false, userLinkColumnsWithDefault...)
	...
	userLink, err := a.UserLinks(tx).All()
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserLinks(tx, false, (*[]*User)(&slice)); err != nil {
		t.Fatal(err)
	}
	...
	a.R.UserLinks = nil
	if err = a.L.LoadUserLinks(tx, true, &a); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserLinks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpUserLinks(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		count, err := a.UserLinks(tx).Count()
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpUserLinks(t *testing.T) {
	var err error
	...
	err = a.SetUserLinks(tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetUserLinks(tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.UserLinks[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.UserLinks[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpUserLinks(t *testing.T) {
	var err error
	...
	err = a.AddUserLinks(tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveUserLinks(tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.UserLinks) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.UserLinks[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.UserLinks[0] != &e {
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Links`: `character varying`, `Login`: `character varying`, `Password`: `character varying`}
	_           = bytes.MinRead
)
